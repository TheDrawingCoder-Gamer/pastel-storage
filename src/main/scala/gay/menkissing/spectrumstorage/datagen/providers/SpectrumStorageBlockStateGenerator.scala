...
final class SpectrumStorageBlockStateGenerator(output: PackOutput, existingFileHelper: ExistingFileHelper) extends BlockStateProvider(output, SpectrumStorage.ModId, existingFileHelper) {
  private class ShelfGenerator(val block: Block):
    import SpectrumStorageBlockStateGenerator.*
    ...
    def generateBottomlessShelfModels(): Unit =
      val baseModel = models().getExistingFile(SpectrumStorage.locate("block/bottomless_shelf"))
  ...
  override protected def registerStatesAndModels(): Unit =
    val shelfGenerator = ShelfGenerator(SpectrumStorageBlocks.bottomlessShelf.get())
    shelfGenerator.generateBottomlessShelfModels()


    barrelBlock(SpectrumStorageBlocks.bottomlessBarrel.get())
    barrelBlock(SpectrumStorageBlocks.bottomlessAmphora.get())

    barrelBlockWithModel(
      SpectrumStorageBlocks.filterChest.get(),
      ExistingModelFile(SpectrumStorageBlocks.filterChest.get().modelLoc, existingFileHelper),
      ExistingModelFile(SpectrumStorageBlocks.filterChest.get().modelLoc.withSuffix("_open"), existingFileHelper)
    )


}

object SpectrumStorageBlockStateGenerator:
  // FUCK YOU OLD FORGE!!!!!!
