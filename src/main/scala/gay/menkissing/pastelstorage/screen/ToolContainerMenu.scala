...
class ToolContainerMenu(windowId: Int, playerInv: Inventory, val box: ItemStack) extends AbstractContainerMenu(PastelStorageScreens.toolContainer.get(), windowId):
  locally:
    val boxInv =
  ...
  def isValidItem(item: ItemStack): Boolean = item.is(PastelStorageTags.item.validToolTag)
