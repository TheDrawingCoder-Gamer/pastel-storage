...
import gay.menkissing.pastelstorage.content.block.entity.FilterChestBlockEntity
...
class FilterChestMenu(windowId: Int, playerInv: Inventory, val container: Container, val blockEntity: FilterChestBlockEntity | Null, val data: FilterConfigurable.ExtendedDataWithPos) extends AbstractContainerMenu(PastelStorageScreens.filterChest.get(), windowId):
